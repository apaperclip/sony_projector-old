...
This document describes the technical architecture of the SonySDCP custom component for Home Assistant.
...
│   ├── __init__.py          # Exports SonySDCPDataUpdateCoordinator
...
│   ├── __init__.py          # Exports SonySDCPEntity
...
- `base.py` - Main coordinator class (`SonySDCPDataUpdateCoordinator`)
...
**Key class:** `SonySDCPDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `SonySDCPConfigFlowHandler` (main flow)
- `SonySDCPOptionsFlow` (options)
...
**Key class:** `SonySDCPEntity` (in `entity/base.py`)
...
2. `SonySDCPEntity` for common functionality
...
3. Create entity classes inheriting from platform base + `SonySDCPEntity`
